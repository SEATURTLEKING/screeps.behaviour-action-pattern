...
mod.handleFlagRemoved = flagName => {
    ...
    if( flagMem && flagMem.task === mod.name && flagMem.roomName ){
        ...
        let flags = FlagDir.filter(FLAG_COLOR.invade.powerMining, new RoomPosition(25,25,flagMem.roomName), true);
...
mod.handleSpawningStarted = params => {
    if ( !params.destiny || !params.destiny.task || params.destiny.task != mod.name )
        return;
    let memory = Task.powerMining.memory(params.destiny.room);
...
mod.handleSpawningCompleted = creep => {
    ...
    // get task memory
    let memory = Task.powerMining.memory(creep.data.destiny.room);
